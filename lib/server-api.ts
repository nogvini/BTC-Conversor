...
export async function fetchAllAppData(): Promise<AppData> {
  ...
  try {
    ...
    const historicalUsdResult = await getHistoricalData('usd', 30);
    const historicalDataUSD = historicalUsdResult?.data || [];
    const historicalBrlResult = await getHistoricalData('brl', 30);
    const historicalDataBRL = historicalBrlResult?.data || [];

    appData = {
      currentPrice: currentPrice,
      historicalData: {
        usd: {
          '30d': historicalDataUSD
        },
        brl: {
          '30d': historicalDataBRL
        }
...
async function refreshCacheInBackground(currency = 'usd', days = 30): Promise<void> {
  ...
  setTimeout(async () => {
    try {
      ...
      // Buscar novos dados usando getHistoricalData (função moderna)
      const historicalResult = await getHistoricalData(currency, days);
      ...
      if (cacheData) {
        if (currency === 'usd') {
          cacheData.historicalDataUSD = data;
        } else {
          cacheData.historicalDataBRL = data;
        }
...
export async function forceUpdateAllData(): Promise<AppData> {
  try {
    ...
    try {
      ...
      const historicalUsdResult = await getHistoricalData('usd', 30);
      historicalDataUSD = historicalUsdResult?.data || [];
