...
||||||| parent of 2e201a6 (refactor: expandir o ProfitCalculator com novas funcionalidades e melhorias na interface, incluindo a adição de novos estados e diálogos para gerenciamento de relatórios, opções de exportação avançada e validação de dados, otimizando a experiência do usuário ao registrar e visualizar informações financeiras.)
=======
...
import { useToast } from "@/components/ui/use-toast"; // Adicionar importação para useToast
...
export default function ProfitCalculator({ btcToUsd, brlToUsd, appData }: ProfitCalculatorProps) {
  ...
  const notificationToastRef = React.useRef<ReturnType<typeof toast> | null>(null);
  const [currentToastId, setCurrentToastId] = React.useState<string | null>(null);
...
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { toast, dismiss as dismissToast } from "@/components/ui/use-toast";
import { format, subMonths, addMonths, startOfMonth, endOfMonth, isWithinInterval, isBefore, startOfDay, endOfDay, differenceInDays } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { cn } from "@/lib/utils";
...
export default function ProfitCalculator({ btcToUsd, brlToUsd, appData }: ProfitCalculatorProps) {
  ...
  const { toast: newToast, dismiss: dismissToast } = useToast(); // Obter toast e dismiss do hook
  const notificationToastRef = React.useRef<ReturnType<typeof newToast> | null>(null); // Referência para o toast de notificação
  const [currentToastId, setCurrentToastId] = React.useState<string | null>(null);
  ...
  const handleAddInvestmentButtonClick = () => { // RENOMEADO para evitar conflito
    ...
    if (!targetReportId) {
      if (!allReportsFromHook || allReportsFromHook.length === 0) {
         addReport("Relatório Padrão");
         newToast({ title: "Relatório Criado", description: "Um 'Relatório Padrão' foi criado. Tente adicionar o aporte novamente.", variant: "default" });
         return;
      } else if (allReportsFromHook.length > 0 && !activeReportIdFromHook) {
        selectReport(allReportsFromHook[0].id);
        targetReportId = allReportsFromHook[0].id;
        newToast({ title: "Relatório Ativado", description: `Relatório "${allReportsFromHook[0].name}" ativado. Tente adicionar o aporte novamente.`, variant: "default" });
        return;
      } else {
         newToast({ title: "Nenhum relatório ativo", description: "Por favor, selecione um relatório ou crie um novo.", variant: "warning" });
        return;
      }
    }
    ...
    const reportToUpdate = allReportsFromHook?.find(r => r.id === targetReportId);
    if (!reportToUpdate) {
        newToast({ title: "Erro", description: "Relatório alvo não encontrado para adicionar aporte.", variant: "destructive" });
        return;
    }
  ...
  const confirmAddInvestment = async (investmentBaseData: Omit<Investment, "id" | "priceAtDate" | "priceAtDateCurrency" | "priceAtDateSource">) => {
    if (!activeReportIdFromHook) {
      newToast({ title: "Erro", description: "Nenhum relatório ativo para adicionar o aporte.", variant: "destructive" });
      return;
    }
    // A função addInvestment do hook já lida com a adição ao relatório ativo
    const success = addInvestment(investmentBaseData); 
  ...
  const handleAddProfitRecordButtonClick = () => { // RENOMEADO para evitar conflito
    ...
    if (!targetReportId) {
      if (!allReportsFromHook || allReportsFromHook.length === 0) {
         addReport("Relatório Padrão");
         newToast({ title: "Relatório Criado", description: "Um 'Relatório Padrão' foi criado. Tente adicionar o registro novamente.", variant: "default" });
         return;
      } else if (allReportsFromHook.length > 0 && !activeReportIdFromHook) {
        selectReport(allReportsFromHook[0].id);
        targetReportId = allReportsFromHook[0].id;
        newToast({ title: "Relatório Ativado", description: `Relatório "${allReportsFromHook[0].name}" ativado. Tente adicionar o registro novamente.`, variant: "default" });
        return;
      } else {
         newToast({ title: "Nenhum relatório ativo", description: "Por favor, selecione um relatório ou crie um novo.", variant: "warning" });
        return;
    ...
    const reportToUpdate = allReportsFromHook?.find(r => r.id === targetReportId);
     if (!reportToUpdate) {
        newToast({ title: "Erro", description: "Relatório alvo não encontrado para adicionar lucro/perda.", variant: "destructive" });
        return;
    }
  ...
  const confirmAddProfitRecord = (profitData: Omit<ProfitRecord, "id">) => { // Recebe Omit<ProfitRecord, "id">
    if (!activeReportIdFromHook) {
      newToast({ title: "Erro", description: "Nenhum relatório ativo para adicionar o registro.", variant: "destructive" });
      return;
    }
  ...
  const deleteInvestmentLocal = (id: string) => {
    if (!activeReportIdFromHook) { 
      newToast({ title: "Erro", description: "Nenhum relatório ativo selecionado.", variant: "destructive" });
      return;
    }
  ...
  const deleteProfitLocal = (id: string) => {
    if (!activeReportIdFromHook) { 
      newToast({ title: "Erro", description: "Nenhum relatório ativo selecionado para exclusão.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (options.periodSelectionType === 'specificMonth' && options.specificMonthDate) {
        ...
        if (isBefore(rangeEnd, rangeStart)) {
           newToast({ title: "Erro no Período", description: "A data final do intervalo não pode ser anterior à data inicial.", variant: "destructive" });
           setIsExporting(false);
           return;
        }
        finalInvestmentsToExport = investmentsToExport.filter(inv => {
        ...
        if (historyFilterType === 'month') {
          ...
          const histEnd = endOfDay(customEndDate);
           if (isBefore(histEnd, histStart)) {
             newToast({ title: "Erro no Período do Histórico", description: "A data final do filtro de histórico não pode ser anterior à inicial.", variant: "destructive" });
             setIsExporting(false);
             return;
          }
      ...
      if (options.exportFormat === 'excel') {
        ...
        if (options.includeSummarySection) {
            ...
            const { operationalProfitBtc: opProfitExportBtc, operationalProfitSats: opProfitExportSats } = calculateOperationalProfitForSummary(
            ...
            summarySheet.addRow({ metric: 'Lucro de Operações (SATS)', value: opProfitExportSats });
            if (appData?.currentPrice) {
  ...
  const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!currentActiveReportForCsv) {
      newToast({ title: "Erro na Importação", description: "Não foi possível encontrar o relatório ativo.", variant: "destructive" });
      if (csvFileInputRef.current) csvFileInputRef.current.value = '';
      return;
    }
...
export default function ProfitCalculator({ btcToUsd, brlToUsd, appData }: ProfitCalculatorProps) {
  ...
  const reportSummaryData = useMemo(() => {
    ...
    const totalActiveInvestmentsCount = investments.filter(inv => inv.priceAtDate !== undefined && inv.priceAtDate !== null && inv.priceAtDate > 0).length;
      ...
      >
        ...
        <TabsContent value="history" className="mt-4">
          ...
          <Card className="bg-black/30 border border-purple-700/40 shadow-xl shadow-purple-900/10 rounded-lg">
            ...
            <CardContent>              {/* Exibir lucro operacional em BTC e Satoshis */}              {currentActiveReportObjectFromHook?.profits && currentActiveReportObjectFromHook.profits.length > 0 && (                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-card p-4 rounded-lg">                  <div className="bg-background/50 p-4 rounded-lg">                    <h3 className="text-sm font-medium mb-2">Lucro Operacional Bruto</h3>                    <div className="flex flex-col">                      <span className="text-lg font-bold">                        {formatCryptoAmount(                          calculateOperationalProfitForSummary(                            currentActiveReportObjectFromHook.profits,                            convertToBtc                          ).operationalProfitBtc,                          "BTC"                        )}                      </span>                      <span className="text-sm text-muted-foreground">                        {Math.round(                          calculateOperationalProfitForSummary(                            currentActiveReportObjectFromHook.profits,                            convertToBtc                          ).operationalProfitSats                        ).toLocaleString()} sats                      </span>                    </div>                  </div>                                    <div className="bg-background/50 p-4 rounded-lg">                    <h3 className="text-sm font-medium mb-2">Lucro Operacional Líquido</h3>                    <div className="flex flex-col">                      <span className="text-lg font-bold">                        {formatCryptoAmount(                          calculateOperationalProfitForSummary(                            currentActiveReportObjectFromHook.profits,                            convertToBtc                          ).netProfitFromOperationsBtc,                          "BTC"                        )}                      </span>                      <span className="text-sm text-muted-foreground">                        {Math.round(                          calculateOperationalProfitForSummary(                            currentActiveReportObjectFromHook.profits,                            convertToBtc                          ).netProfitFromOperationsSats                        ).toLocaleString()} sats                      </span>                    </div>                  </div>                                    <div className="bg-background/50 p-4 rounded-lg">                    <h3 className="text-sm font-medium mb-2">Saldo Total Estimado</h3>                    <div className="flex flex-col">                      {currentActiveReportObjectFromHook.investments && (                        <>                          <span className="text-lg font-bold">                            {formatCryptoAmount(                              calculateAverageBuyPriceForSummary(                                currentActiveReportObjectFromHook.investments,                                brlToUsd,                                convertToBtc                              ).totalInvestmentsBtc +                               calculateOperationalProfitForSummary(                                currentActiveReportObjectFromHook.profits,                                convertToBtc                              ).netProfitFromOperationsBtc,                              "BTC"                            )}                          </span>                          <span className="text-sm text-muted-foreground">                            {Math.round(                              (calculateAverageBuyPriceForSummary(                                currentActiveReportObjectFromHook.investments,                                brlToUsd,                                convertToBtc                              ).totalInvestmentsBtc +                               calculateOperationalProfitForSummary(                                currentActiveReportObjectFromHook.profits,                                convertToBtc                              ).netProfitFromOperationsBtc) * 100000000                            ).toLocaleString()} sats                          </span>                        </>                      )}                    </div>                  </div>                </div>              )}                            {showFilterOptions && (                <div className="px-0 pt-4 pb-2 border-t border-purple-700/20 mt-3">                  {historyFilterType === 'month' && (
                    <div className="flex items-center space-x-2">
...
>>>>>>> 2e201a6 (refactor: expandir o ProfitCalculator com novas funcionalidades e melhorias na interface, incluindo a adição de novos estados e diálogos para gerenciamento de relatórios, opções de exportação avançada e validação de dados, otimizando a experiência do usuário ao registrar e visualizar informações financeiras.)
